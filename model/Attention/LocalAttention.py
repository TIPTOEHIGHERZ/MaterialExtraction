...
class LocalAttention:
    ...
    def prepare_input(self, hidden_states: torch.Tensor, heads: int):
        ...
        hidden_states = hidden_states.reshape(b, hidden_dim, -1, self.patch_size, self.patch_size)
        hidden_states = hidden_states.reshape(*hidden_states.shape[:3], -1).transpose(1, 2)
        hidden_states = hidden_states.reshape(-1, *hidden_states.shape[2:]).transpose(-1, -2)

        # [b * n * n, patch_size * patch_size, hidden_dim]
        hidden_states = hidden_states.reshape(*hidden_states.shape[:2], hidden_dim // heads, heads)
    ...
    def prepare_output(self, hidden_states: torch.Tensor, heads=None):
        ...
        # [b * n * n, patch_size * patch_size, hidden_dim]
        b = int(hidden_states.shape[0] / (self.n ** 2))
        hidden_dim = hidden_states.shape[-1]

        hidden_states = hidden_states.reshape(b, -1, self.patch_size ** 2, hidden_dim)
