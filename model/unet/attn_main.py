...
from diffusers.models.unets.unet_2d_blocks import ResnetBlock2D, DualTransformer2DModel, Upsample2D
...
from diffusers.models.normalization import AdaLayerNormSingle, AdaLayerNormContinuous, AdaLayerNorm, AdaLayerNormZero
from diffusers.models.attention import Attention, FeedForward, GatedSelfAttentionDense, _chunked_feed_forward
...
def get_down_block(
    ...
    if down_block_type == "DownBlock2D":
        ...
        return CrossAttnDownBlock2DMain(
            num_layers=num_layers,
...
def get_mid_block(
    ...
    if mid_block_type == "UNetMidBlock2DCrossAttn":
        return UNetMidBlock2DCrossAttnMain(
            transformer_layers_per_block=transformer_layers_per_block,
            in_channels=in_channels,
            temb_channels=temb_channels,
            dropout=dropout,
...
class BasicTransformerBlockMain(nn.Module):
    r"""
    ...
    def forward(
        ...
        attn2_hidden_states = None if attn_queue is None else attn_queue[0]
        # attn2_hidden_states = encoder_hidden_states

        # TODO when apply lora on unet_ref, hidden_states shouldn't be ignored
        # if isinstance(self.attn1.processor, AttnProcessor2_0):
        #     attn1_hidden_states = None
        #     attention_mask = None
        ...
        if self.attn2 is not None:
            ...
            if encoder_attention_mask is not None:
                ...
                encoder_attention_mask = nn.functional.interpolate(encoder_attention_mask, [int(attn2_hidden_states.shape[1] ** 0.5)] * 2)
                encoder_attention_mask = (1 - encoder_attention_mask) * -10000.
                encoder_attention_mask = encoder_attention_mask.view(*encoder_attention_mask.shape[:2], -1)
            
            attn_output = self.attn2(
                norm_hidden_states,
                encoder_hidden_states=attn2_hidden_states,
...
class Transformer2DModelMain(LegacyModelMixin, LegacyConfigMixin):
    """
    ...
    def forward(
        ...
        for block in self.transformer_blocks:
            attn_states = None if attn_queue is None else attn_queue.pop(0)
...
class CrossAttnUpBlock2DMain(nn.Module):
    def __init__(
        self,
        in_channels: int,
        out_channels: int,
...
class CrossAttnDownBlock2DMain(nn.Module):
    def __init__(
        self,
        in_channels: int,
        out_channels: int,
...
class UNetMidBlock2DCrossAttnMain(nn.Module):
    def __init__(
        self,
        in_channels: int,
        temb_channels: int,
