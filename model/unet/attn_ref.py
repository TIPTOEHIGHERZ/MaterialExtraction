...
def get_down_block(
    ...
    if down_block_type == "DownBlock2D":
        ...
        return CrossAttnDownBlock2DReference(
            num_layers=num_layers,
...
def get_mid_block(
    ...
    if mid_block_type == "UNetMidBlock2DCrossAttn":
        return UNetMidBlock2DCrossAttnReference(
            transformer_layers_per_block=transformer_layers_per_block,
            in_channels=in_channels,
            temb_channels=temb_channels,
            dropout=dropout,
...
def get_up_block(
    ...
    if up_block_type == "UpBlock2D":
        ...
        return CrossAttnUpBlock2DReference(
            num_layers=num_layers,
            transformer_layers_per_block=transformer_layers_per_block,
            in_channels=in_channels,
            out_channels=out_channels,
    ...
    def set_use_memory_efficient_attention_xformers(
        ...
        if use_memory_efficient_attention_xformers:
            ...
            if not is_xformers_available():
                ...
                raise NotImplementedError('no xforms is installed')
                # try:
...
@maybe_allow_in_graph
class BasicTransformerBlockReference(nn.Module):
    r"""
    ...
    def forward(
        ...
        attn_output, attn1_states = self.attn1(
            norm_hidden_states,
            encoder_hidden_states=encoder_hidden_states if self.only_cross_attention else None,
            attention_mask=attention_mask,
            **cross_attention_kwargs,
        )
        attn1_states = attn1_states if self.enable_cross else None
        ...
        if self.attn2 is not None:
            ...
            attn_output, attn2_states = self.attn2(
                norm_hidden_states,
                encoder_hidden_states=encoder_hidden_states,
        ...
        if self.attn2 is not None:
            return hidden_states, (attn1_states, attn2_states)
        
        return hidden_states, (attn1_states,)


class Transformer2DModelReference(LegacyModelMixin, LegacyConfigMixin):
    """
    ...
    def forward(
        ...
        for block in self.transformer_blocks:
            if torch.is_grad_enabled() and self.gradient_checkpointing:
                hidden_states, attn_states = self._gradient_checkpointing_func(
                    block,
                    hidden_states,
                    attention_mask,
                ...
                hidden_states, attn_states = block(
                    hidden_states,
                    attention_mask=attention_mask,
                    encoder_hidden_states=encoder_hidden_states,
...
class CrossAttnUpBlock2DReference(nn.Module):
    def __init__(
        self,
        in_channels: int,
        out_channels: int,
    ...
    def forward(
        ...
        for resnet, attn in zip(self.resnets, self.attentions):
            ...
            if torch.is_grad_enabled() and self.gradient_checkpointing:
                ...
                hidden_states, attn_states = attn(
                    hidden_states,
                    encoder_hidden_states=encoder_hidden_states,
                    cross_attention_kwargs=cross_attention_kwargs,
                    attention_mask=attention_mask,
        ...
        return hidden_states, attn_queue
    

class CrossAttnDownBlock2DReference(nn.Module):
    def __init__(
        self,
        in_channels: int,
        out_channels: int,
    ...
    def forward(
        ...
        return hidden_states, output_states, attn_queue
    

class UNetMidBlock2DCrossAttnReference(nn.Module):
    def __init__(
        self,
        in_channels: int,
        temb_channels: int,
