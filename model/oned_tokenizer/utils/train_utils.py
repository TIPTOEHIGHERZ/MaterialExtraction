...
def create_dataloader(config, logger, accelerator):
    ...
    # potentially, use a pretokenized dataset for ImageNet speed-up.
    elif dataset_config.get("pretokenization", ""):
        ...
        train_dataloader = DataLoader(
            LabelDataLoader('./imagenet', transforms=transform),
            batch_size=config.training.per_gpu_batch_size
        )
...
def train_one_epoch(config, logger, accelerator,
    ...
    for i, batch in enumerate(train_dataloader):
        ...
        if accelerator.sync_gradients:
            ...
            # Generate images.
            if (global_step + 1) % config.experiment.generate_every == 0 and accelerator.is_main_process and model_type == "tatitok":
