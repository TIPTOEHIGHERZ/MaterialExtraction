...
class PerceptualLoss(torch.nn.Module):
    def __init__(self, model_name: str = "convnext_s"):
        ...
        if "convnext_s" in model_name:
            self.convnext = models.convnext_small(weights=False, pretrained=False).eval()
            self.convnext.load_state_dict(torch.load('./pretrained/convnext_small-0c510722.pth', weights_only=True))
