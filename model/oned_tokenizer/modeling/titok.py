...
# from model.oned_tokenizer.modeling.modules.base_model import BaseModel
# from model.oned_tokenizer.modeling.modules.blocks import TiTokEncoder, TiTokDecoder
# from model.oned_tokenizer.modeling.quantizer.quantizer import VectorQuantizer, DiagonalGaussianDistribution
# from model.oned_tokenizer.modeling.modules.maskgit_vqgan import Encoder as Pixel_Eecoder
# from model.oned_tokenizer.modeling.modules.maskgit_vqgan import Decoder as Pixel_Decoder
# from model.oned_tokenizer.modeling.modules.maskgit_vqgan import VectorQuantizer as Pixel_Quantizer
...
class TiTok(BaseModel, PyTorchModelHubMixin, tags=["arxiv:2406.07550", "image-tokenization"], repo_url="https://github.com/bytedance/1d-tokenizer", license="apache-2.0"):
    def __init__(self, config):
        ...
        if self.finetune_decoder:
            ...
            self.pixel_decoder = Pixel_Decoder(OmegaConf.create(
                    ...
                    "resolution": 256,
                    "z_channels": 256}))
        elif config.model.vq_model.get('vq_gan_ckpt', ''):
            ckpt_path = config.model.vq_model.vq_gan_ckpt
            tokenizer = PretrainedTokenizer(ckpt_path)
            self.pixel_decoder = tokenizer.decoder
            self.pixel_quantize = tokenizer.quantize
            self.pixel_encoder = tokenizer.encoder

            self.pixel_encoder.eval()
            self.pixel_decoder.eval()
            self.pixel_quantize.eval()

            self.pixel_encoder.requires_grad_(False)
            self.pixel_decoder.requires_grad_(False)
            self.pixel_quantize.requires_grad_(False)
    ...
    def encode(self, x, attention_mask: torch.Tensor=None):
        if self.finetune_decoder:
            with torch.no_grad():
                self.encoder.eval()
                self.quantize.eval()
                z = self.encoder(pixel_values=x, latent_tokens=self.latent_tokens, attention_mask=attention_mask)
    ...
    def forward(
        ...
        z_quantized, result_dict = self.encode(x, attention_mask=attention_mask)
        decoded = self.decode(z_quantized)

        if not self.finetune_decoder:
            hidden_states = self.pixel_encoder(x[:, :3]) if x_gt is None else self.pixel_encoder(x_gt[:, :3])
            quantized_states, codebook_indices, codebook_loss = self.pixel_quantize(hidden_states)

            reconstruction_loss = nn.functional.cross_entropy(decoded.view(*decoded.shape[:2], -1), codebook_indices)
            result_dict['reconstruct_loss'] = reconstruction_loss
            result_dict['total_loss'] = reconstruction_loss + result_dict['quantizer_loss']
