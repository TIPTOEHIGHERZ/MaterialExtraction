...
from scripts.finetune_concat import evaluate, prepare_model, transform
...
config = omegaconf.OmegaConf.load('./configs/finetune_concat.yaml')
...
save_dir = os.path.join('eval', f'{time.strftime("concat_%Y-%m-%d %H:%M:%S", current_time)}_{cond_scale: .3f}_cfg')
