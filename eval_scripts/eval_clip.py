...
from scripts.finetune_clip import evaluate, prepare_model, transform
...
config = omegaconf.OmegaConf.load('./configs/finetune_clip.yaml')
...
save_dir = os.path.join('eval', f'{time.strftime("clip_%Y-%m-%d %H:%M:%S", current_time)}_{cond_scale: .3f}_cfg')
