...
def training_step(module: list[nn.Module], data, pipeline: Pipeline, device='cuda'):
    ...
    theta = data['meta_data']['theta']
    data['meta_data']['theta'] = [(t - r / 180 * np.pi) for t, r in zip(theta, rotate_angels)]
    # align rotation
    color, normal, height, roughness = align_attr(color, normal, height, roughness, data['meta_data'], functools.partial(transform, res=512))
    # color, normal, height, roughness = torchvision.transforms.RandomCrop(256)(torch.concat([color, normal, height, roughness], dim=1)).split([3, 3, 3, 3], dim=1)
...
def prepare_model(
    ...
    PBRTextureDataLoader.image_names = ('image_project', 'mask_project', 'meta_data')
