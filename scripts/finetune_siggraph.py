...
class FeatureEncoder(PartialSAEncoder):
    def __init__(
        ...
        super().__init__(in_channels, context_dim, key, multi_channel=False)
...
def log_function(
    ...
    if global_iter % log_period_visual == 0:
        encoder_hidden_states = feature_encoder((image_ref - 0.5) * 2, mask=mask_ref)

        masked_ref_latents = pipeline.image2latents(image_ref * mask_ref, mask=mask_ref)
        eval_result = evaluate(
            pipeline,
            masked_ref_latents,
            encoder_hidden_states,
            [s // 8 for s in image_ref.shape[-2:]],
            image_ref.device,
            ...
            logger.add_image(
                ...
                torch.concat([
                    torch.concat([
                        ...
                        zip(eval_color, color, image_ref)
                ], dim=-2),
                global_iter
            )
    return
    ...
    **kwargs
    ...
    if global_iter % log_period_loss == 0:
        for i in range(num_samples) if logger is None else tqdm.trange(num_samples, leave=False):
            data = dataset[idx]
            image_ref, mask_ref, color, normal, height, roughness = data['image_nolight'], data['mask'], data['Color'], data['NormalGL'], data['Height'], data['Roughness']
            ...
            mask_ref = mask_ref * nn.functional.interpolate(torch.tensor(random_mask(512), device=device).unsqueeze(0), mask_ref.shape[-2:])
            # augment = tps(torch.concat([image_ref, mask_ref], dim=1))
            ...
            masked_ref_latents = pipeline.image2latents(image_ref * mask_ref, mask_ref)
            
            image_ref: torch.Tensor = (image_ref - 0.5) * 2
            image_ref *= mask_ref
          
            encoder_hidden_states = feature_encoder(image_ref, mask=mask_ref)
            ...
            logger.add_image(
                ...
                torch.concat([
                    torch.concat([
                        ...
                        zip(eval_color, color, (image_ref + 1) / 2)
                ], dim=-2),
                global_iter
            )
...
def training_step(module: list[nn.Module], data, pipeline: Pipeline, device='cuda'):
    ...
    mask_ref = mask_ref * nn.functional.interpolate(torch.tensor(random_mask(512), device=device).unsqueeze(0), mask_ref.shape[-2:])
    # augment = tps(torch.concat([image_ref, mask_ref], dim=1))
    # image_ref = augment[:, :image_ref.shape[1], ...]
    # mask_ref = augment[:, image_ref.shape[1]:, ...][:, :1, ...]
    ...
    encoder_hidden_states = feature_encoder((image_ref * 2) - 1., mask=mask_ref)
...
def transform(x: torch.Tensor):
    ...
    x = torchvision.transforms.RandomCrop(448)(x)
...
def prepare_model(
    ...
    feature_encoder = FeatureEncoder(3, 768, image_dim=3136, cross_attn_dim=768)
    ...
    unet = UNet2DConditionModel.from_config(UNet2DConditionModel.load_config('./configs/unet/siggraph/config_main.json'))
    pipeline.unet = unet
    ...
    params = [
        ...
        {'params': list(feature_encoder.parameters()), 'lr': lr * lr_gain[1]},
    ]
    ...
    PBRTextureDataLoader.image_names = ('image_nolight', 'mask')
    ...
    data_loader = PBRTextureDataLoader(
        ...
        transform_group={'default': ['Color', 'NormalGL', 'Height', 'Roughness', 'image_nolight', 'mask'], 'no_transform': []}
    )

    val_loader = PBRTextureDataLoader(
        fp='./datasets/render_result_matsynth_10_resized', 
        gt_fp='./datasets/MatSynth/textures_all_resized', 
        transforms={'default': transform, 'no_transform': lambda x: x},
        fetch_attr=('Color', 'NormalGL', 'Height', 'Roughness'),
    ...
    return module, data_loader, None, val_loader, optimizer, start_epoch, pipeline
