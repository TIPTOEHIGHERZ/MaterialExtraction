...
def training_step(module: dict[nn.Module], data, pipeline: Pipeline, device='cuda'):
    ...
    color, normal, height, roughness = transform(color, 256), transform(normal, 256), transform(height, 256), transform(roughness, 256)
...
def prepare_model(
    ...
    data_loader = PBRTextureDataLoader(
        ...
        fetch_attr=('Color', 'NormalGL', 'Roughness', 'Height'),
        gt_mapping={
            'diff_4k': 'Color',
            'nor_gl_4k': 'NormalGL',
            'rough_4k': 'Roughness',
