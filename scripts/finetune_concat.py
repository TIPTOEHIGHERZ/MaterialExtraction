...
def evaluate(
    ...
    for t in timesteps:
        ...
        latents_input = torch.concat([masked_ref_latents, latents_start], dim=1)

        encoder_hidden_states_input = encoder_hidden_states
        if cond_scale is not None:
            latents_input_null = torch.zeros_like(latents_input)
            latents_input = torch.concat([latents_input, latents_input_null], dim=0)
            encoder_hidden_states_input = torch.concat([encoder_hidden_states_input, encoder_hidden_states_input], dim=0)
    ...
    **kwargs
    ...
    if global_iter % log_period_loss == 0:
        for i in range(num_samples) if logger is None else tqdm.trange(num_samples, leave=False):
            ...
            noised_latents = torch.concat([masked_ref_latents, noised_latents], dim=1)
...
def training_step(module: list[nn.Module], data, pipeline: Pipeline, device='cuda'):
    ...
    image_ref: torch.Tensor = (image_ref - 0.5) * 2
    image_ref *= mask_ref
    masked_ref_latents = pipeline.image2latents(image_ref, mask=mask_ref)
...
def prepare_model(
    ...
    # )
    unet = UNet2DConditionModel.from_config(UNet2DConditionModel.load_config('configs/unet/concat/config_main.json'))
    # unet.unet_ref.load_state_dict(pipeline.unet.state_dict())
    try_load(unet, pipeline.unet.state_dict())
    pipeline.unet = unet
    # frozen parameters
    pipeline.frozen()
    ...
    memory_halt.halt(int(15 - torch.cuda.memory_allocated() / 1024 ** 3 - 2))
