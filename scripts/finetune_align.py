...
def log_function(
    ...
    pipeline: Pipeline,
    color: torch.Tensor,
    vision_transformer: CLIPVisionTransformer,
    ...
    if global_iter % log_period_visual == 0:
        ...
        encoder_hidden_states = vision_transformer(clip_input, return_dict=True).last_hidden_state[:, 1:, ...]
...
def training_step(module: list[nn.Module], data, pipeline: Pipeline, clip_processor: CLIPProcessor, device='cuda'):
    unet = module['unet']
    vision_transformer = module['clip']
    ...
    color = torchvision.transforms.functional.rotate(color, flip_angle)
...
def prepare_model(
    ...
    vision_transformer_param = list()
    for name, param in frozen_vision_transformer.named_parameters():
        if name in ('post_layernorm.bias', 'post_layernorm.weight'):
            param.requires_grad_(False)
        else:
            param.requires_grad_(True)
            vision_transformer_param.append(param)
    ...
    params = [
        ...
        {'params': list(vision_transformer_param), 'lr': lr * lr_gain[1]},
    ]
    ...
    module = {
        'unet': unet,
        'clip': frozen_vision_transformer
